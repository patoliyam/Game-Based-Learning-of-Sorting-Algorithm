...
public class SwapCubesBubbleSort : MonoBehaviour
{
...
IEnumerator Swap(float speed, GameObject go1, GameObject go2)
    ...
    IEnumerator ValidateSwap()
    {
        ...
        valid = GameController.GetComponent<GameBrainBubbleSort>().ValidateMove(index);

        GameController.GetComponent<GameBrainBubbleSort>().GameOver();
    ...
    IEnumerator UpdateScore()
    {
        ...
        GameController.GetComponent<GameBrainBubbleSort>().SetScoreText(valid);   
        if (!GameController.GetComponent<GameBrainBubbleSort>().lose)
        {
            if (!valid)
            {
    ...
    {
        if (!(GameController.GetComponent<GameBrainBubbleSort>().won || GameController.GetComponent<GameBrainBubbleSort>().lose))
        {
            foreach (GameObject cube in cubeList)
            {
