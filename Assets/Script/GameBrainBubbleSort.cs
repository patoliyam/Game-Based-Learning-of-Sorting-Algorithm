/*
 * Name of module       : GameBrainBubbleSort 
 * Functions            :
 *                      1. Generate_Cubes()
 *                          Input : None
...
public class GameBrainBubbleSort : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        //Text to be displayed on pressing help button
        helpText.text = "N = Number of cubes\nNumber[ ] = Array to be sorted\nBubblesort(Number[ ], N){\n\tfor i = 1 to N-1\n\t\tfor j = 1 to N-i\n\t\t\tif Number[j] > Number[j+1]\n\t\t\t\tSwap Number[j] and Number[j+1]\n}\nTouch the cubes j and j+1 simultaneously for a valid swap";
        ...
        //Initialling number of consecutive wrong moves before current move with 0
        wrongCount = 0;
        ...
        for (int i = 1; i <= 7; i++)
        {
            for (int j = 1; j <= 8 - i; j++)
            {
                if (Number[j] > Number[j + 1])
                {
                    ...
                    Number[j] = Number[j + 1];
                    Number[j + 1] = temp;
                    moveList.Add(new Move() { first = j, second = j + 1 });
                }
            }
        }
    }
