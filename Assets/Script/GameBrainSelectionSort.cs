/*
 * Name of module       : GameBrainSelectionSort                        
 * Functions            :
 *                      1. Generate_Cubes()
 *                          Input : None
...
public class GameBrainSelectionSort : MonoBehaviour
{
    ...
    public void FindValidMoveList()
    {
        for (int i = 1; i <= 7; i++)
        {
            ...
            if (min_idx != i)
            {
                moveList.Add(new Move() { first = i, second = min_idx });
                int swap_temp = Number[min_idx];
